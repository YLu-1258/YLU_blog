...
Then, I will use a nested for loop to iterate through each individual index in each row of the grid. In this process, I will initialize the value by generating a random from 0-9 with the line `RNG.nextInt(10) >= 6`, with four values (6,7,8,9) reserved for turning the lights on.  
